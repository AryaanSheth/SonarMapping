...
public class display {
     ...
     */
    public JPanel[][] objectMatrix(float[][] matrix){
    ...
    public void generatePanels(float[][] matrix, JFrame frame, JPanel[][] panel, int dimension){
        for(int i = 0; i < matrix.length; i++){
            for(int j = 0; j < matrix.length; j++){
                ...
                panel[i][j].setBounds(j*(dimension/matrix.length), i*(dimension/matrix.length), 
                dimension / matrix[i].length, dimension / matrix.length);

                // defines the color of the JPanel objects in the matrix
                panel[i][j].setBackground(new Color((int) (matrix[i][j] * 150), 
                (int) (matrix[i][j] * 50), (int) (matrix[i][j] * 90)));
