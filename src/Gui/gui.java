...
public class gui{
    ...
    public static void generatePanels(float[][] matrix, JFrame frame, JPanel[][] panel, int dimension){
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                ...
                panel[i][j].setBounds(j*(dimension/matrix[i].length), i*(dimension/matrix.length), dimension/matrix[i].length, dimension/matrix.length);
                panel[i][j].setBackground(new Color((int)(matrix[i][j]*100)+50, (int)(matrix[i][j]*50)+30, (int)(matrix[i][j]*90)+30));
                frame.add(panel[i][j]);
            }
        }
    }
    ...
    public static void main(String[] args){
        ...
        float[][] matrix = {{1,2,3},{4,5,6},{7,8,9}}; // test matrix
        ...
        generatePanels(normMatrix, frame, objectMatrix(matrix), dimension); // call generatePanels method to generate the panels
    }
}