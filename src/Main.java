...
class Main{
    ...
    public static float[][] normalizeMatirx(float[][] matrix, float maxVal){
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                matrix[i][j] = Math.round((matrix[i][j]/maxVal)*100)/100.0f;  // normalize value and round to 2 decimal places
            }
        }
        return matrix;
    }
    
    // prints original matrix vs normalized matrix
    public static void main(String args[]){
        float[][] matrix = {{5,2,5},{7,4,3},{7,8,7}}; // test matrix
        printMatirx(matrix);
        System.out.println();
        printMatirx(normalizeMatirx(matrix, findMaxMatrix(matrix)));
